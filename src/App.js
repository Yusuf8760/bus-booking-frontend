            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-4 p-6 bg-white shadow-lg rounded-lg">
                {seats.filter(seat => seat.deck === 'upper').map(seat => (
                  <button key={seat.id} className={`w-16 h-24 rounded-lg font-bold border transition duration-300 ${seat.is_booked ? "bg-gray-500 text-white cursor-not-allowed" : selectedSeats.includes(seat.id) ? "bg-green-500 text-white" : "bg-gray-300 hover:bg-gray-400"}`} disabled={seat.is_booked} onClick={() => toggleSeatSelection(seat)}>{seat.seat_label}</button>
                ))}
              </div>
            </div>
            <div>
